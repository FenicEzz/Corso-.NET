...
{
    public class CalcolaCodiceFiscaleService : ICalcolaCodiceFiscaleService
    {
        ...
        private string CalcolaGiorno(DateTime giorno, bool gender)
        {
            var sesso = gender;
            var day = giorno.Day;
        ...
        public string CalculateCode(User user)
        {
            string carattereControllo;
            int contoPari = 0;
            int contoDispari = 0;
            var calcolo = CalcolaCognome(user.Surname) +
                CalcolaNome(user.Name) +
                CalcolaAnno(user.Birthdate) +
                CalcolaMese(user.Birthdate) +
                CalcolaGiorno(user.Birthdate, user.IsMale) +
                CalcolaComune(user.Birthcity);
